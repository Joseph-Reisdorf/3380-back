...
export const getClicks = (req, res) => {
    const { listen_to_listener_id, listen_to_album_id } = req.query;
    const q = "SELECT * FROM listen_to WHERE listen_to_listener_id = ? AND listen_to_album_id = ?";
  
    db.query(q, [listen_to_listener_id, listen_to_album_id], (err, data) => {
...
export const showAlbumReport = (req, res) => {
    ...
    const albumClickMonthQuery = `
        SELECT l.listen_to_listener_id, a.album_title, l.listen_to_id, l.listen_to_datetime
        FROM listen_to l
        INNER JOIN album a ON l.listen_to_album_id = a.album_id
        INNER JOIN artist ar ON a.album_primary_artist_id = ar.artist_id
        WHERE ar.artist_id = ? 
